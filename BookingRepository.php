...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $noramlJobs = array();
        if ($cuser && $cuser->is('customer')) {
            $jobs = $cuser->jobs()->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language', 'feedback', 'distance')->whereIn('status', ['completed', 'withdrawbefore24', 'withdrawafter24', 'timedout'])->orderBy('due')->paginate(15);
            $usertype = 'customer';
...
//            $jobs['total'] = $totaljobs;
        ...
        try {
            ...
            if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
                ...
                if (!isset($data['from_language_id'])) {
                    return $this->setResponse("from_language_id");
                }
                if ($data['immediate'] == 'no') {
                    if (isset($data['due_date']) && $data['due_date'] == '') {
                        return $this->setResponse("due_date");
                    }
                    if (isset($data['due_time']) && $data['due_time'] == '') {
                        return $this->setResponse("due_time");
                    }
                    if (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])) {
                        return $this->setResponse("customer_phone_type");
                    }
                    if (isset($data['duration']) && $data['duration'] == '') {
                        return $this->setResponse("duration");
                    }
                } else {
                    if (isset($data['duration']) && $data['duration'] == '') {
                        return $this->setResponse("duration");
    ...
    private function setResponse($fieldName) {
        $response['status'] = config('constants.FAIL');
        $response['message'] = config('constants.FAIL_MSG');
...
//        }
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if ($current_translator || (isset($data['translator']) && $data['translator']) || $data['translator_email'] != '') {
            $log_data = [];
